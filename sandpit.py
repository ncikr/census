...
census_data_top_areas = census_data_top_areas.sort_values('MSOA %', ascending=False).head(10)
...
chart.animate(Data.filter(area_filter),
              Config({
                  "title": "...but with notable the top 30",
                  "sort": "byValue",
                }
                ),
                Style(
...
filter_tory_votes = Data.filter(
    """
    record['Conservative % Share'] == 'Gay or Lesbian' || 
    record['Sexual Orientation'] == 'Gay or Lesbian' || 
    record['Sexual Orientation'] == 'Bisexual' || 
    record['Sexual Orientation'] == 'Other'
    """
...
chart.animate(
    filter_lgb,
    Config({
            "title": "There was no correlation found between % LGB+ and the likelyhood to vote for the Conservative party",                      
            "geometry":"circle",
            "channels": {
                    "x": {"set":'mean(MSOA %)'},
